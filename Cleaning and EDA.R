...
# Step 1: Exclude unwanted variables
# Identify character columns
cat_cols <- names(train)[sapply(train, is.character)]

# Remove binary character columns
non_binary_cat_cols <- cat_cols[sapply(train[cat_cols], function(col) length(unique(col)) > 2)]

# Remove 'policy_id' and 'is_claim'
non_binary_cat_cols <- setdiff(non_binary_cat_cols, c("policy_id", "is_claim"))

# Step 2: Plot each as bar chart
for (col in non_binary_cat_cols) {
  print(
    ggplot(train, aes_string(x = col)) +
